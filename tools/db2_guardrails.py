...
def check_db2_notes(findings: list[Finding]) -> None:
    readme = REPO_ROOT / "README.md"
    if not readme.exists():
        return
    text = read_text(readme)
    if "DB2" not in text:
        add(findings, "WARN", "docs.db2", "README should mention DB2 and how production validations are performed safely.", readme)


def main() -> int:
    parser = argparse.ArgumentParser(description="Offline DB2 governance guardrails for this repo.")
